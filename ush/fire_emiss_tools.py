...
def averaging_FRP(fcst_dates, cols, rows, intp_dir, rave_to_intp, veg_map, tgt_area, beta, fg_to_ug, ebb_dcycle, hwp_alpha):
    ...
    print("EBB DCYCLE 2 selected. HWP_alpha set to:", hwp_alpha) 
    ...
    for cycle in fcst_dates:
        ...
        block_index = (elapsed_hours // 6) % time_blocks if hwp_alpha != 0 else 0
    ...
    for block in range(time_blocks):
        if np.any(data_count_blocks[block]):
            ...
            #num_zeros = len(ebb_smoke_blocks[block]) - np.sum([arr == 0 for arr in ebb_smoke_blocks[block]], axis=0)
            ...
            results.append((temp_frp,ebb_total_reshaped))
        else:
            results.append((np.zeros((cols, rows)), np.zeros((cols, rows))))

        # Stack the FRP and EBB results from all blocks along a new time dimension
    frp_merged = np.stack([res[0] for res in results], axis=0)
    ebb_merged = np.stack([res[1] for res in results], axis=0)

    print('Merged FRP shape:', frp_merged.shape)
    print('Merged EBB shape:', ebb_merged.shape)

    return frp_merged, ebb_merged
...
def save_fire_dur(cols, rows, te, ebb_dcycle, hwp_alpha):
    te_array = np.array(te).reshape(cols, rows)
    if ebb_dcycle == 2 and hwp_alpha != 0.0:
       # Repeat the same values across 4 time frames
        fire_dur = np.repeat(te_array[np.newaxis, :, :], repeats=4, axis=0)
    elif ebb_dcycle == 2 and hwp_alpha == 0.0:
        fire_dur = te_array  # Shape (cols, rows)
    else:
        raise ValueError("Unsupported combination of ebb_dcycle and hwp_alpha")
    return fire_dur
...
def produce_emiss_file(xarr_hwp, frp_avg_reshaped, totprcp_ave_arr, xarr_totprcp, intp_dir, current_day, tgt_latt, tgt_lont, ebb_tot_reshaped, fire_dur, cols, rows, ebb_dcycle,hwp_alpha):
   if ebb_dcycle == 2 and hwp_alpha != 0.0:
       ...
       fire_age = np.clip(fire_dur, 0, None)
       fire_age = np.nan_to_num(fire_age)
       frp_avg_reshaped = xr.DataArray(frp_avg_reshaped)

       fire_age = xr.DataArray(fire_age)
       frp_avg_reshaped = frp_avg_reshaped.where(ebb_tot_reshaped > 0, 0).fillna(0)
       fire_age = fire_age.where(ebb_tot_reshaped > 0, 0).fillna(0)
       ...
       count_filtered_hwp_greater_zero = np.sum(filtered_hwp > 0, axis=(1, 2))
       count_filtered_prcp_greater_zero= np.sum(filtered_prcp  > 0, axis=(1, 2))
       # Produce emiss file
       file_path = os.path.join(intp_dir, f'SMOKE_RRFS_data_{current_day}00.nc')
